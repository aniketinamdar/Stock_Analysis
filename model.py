...
def get_technical_prompt(time_frame, stock_name, exchange):
	technical_prompt = "Act as a stock market expert and perform technical analysis on the company {stock} listed on {xchange}. Use the {tframe} candlestick chart provided below for the analysis. Perform an in-depth technical analysis focusing on single and multiple candlestick patterns, volume, and support - resistance levels. Return the analysis in 3 paragraphs of 80-100 words each.".format(tframe=time_frame, stock=stock_name, xchange=exchange)

	return technical_prompt

def get_intraday_prompt(stock_name, exchange):
	intraday_prompt = "Act as a stock market expert and perform technical analysis on the company {stock} listed on {xchange}. Use the 5 minute and 15 minute candlestick charts provided below for the analysis. Perform an in-depth technical analysis focusing on single and multiple candlestick patterns, volume, and support - resistance levels. Using the analysis, decide whether to take long position or a short position as an intraday trader. For the chosen position, return entry price, stop-loss, target 1 and target 2. While calculating the position, keep the minimum risk reward ratio (risk reward ratio = potential loss / potential profit) above 1. If no suitable position can be found above minimum risk reward ratio, convey the same to the user.".format(stock=stock_name, xchange=exchange)

	return intraday_prompt

def get_swing_prompt(stock_name, exchange):
	swing_prompt = "Act as a stock market expert and perform technical analysis on the company {stock} listed on {xchange}. Use the 1 day and 1 week candlestick charts provided below for the analysis. Perform an in-depth technical analysis focusing on single and multiple candlestick patterns, volume, and support - resistance levels. Using the analysis, decide whether to take a long position as a swing trader. For the chosen position, return entry price, stop-loss, target 1 and target 2. While calculating the position, keep the minimum risk reward ratio (risk reward ratio = potential loss / potential profit) above 1. If no suitable position can be found above minimum risk reward ratio, convey the same to the user.".format(stock=stock_name, xchange=exchange)
